...
// Full-spectrum thermal colour stops: black → blue → cyan → green → yellow → orange → red → white
const THERMAL_STOPS = [
  [0,    [0,   0,   0  ]],
  [0.13, [0,   0,   128]],
  [0.28, [0,   0,   255]],
...
/**
 * Map a [0,1] luminance value to an RGB thermal colour.
...
// 8×8 Bayer ordered dither matrix, normalised to [-0.5, 0.5]
const BAYER8 = [
...
/**
 * Apply Bayer-dithered thermal colour-map to a <video> source
 * and render the result onto destCanvas, including HUD overlays.
 *
 * @param {HTMLVideoElement} srcVideo
 * @param {HTMLCanvasElement} destCanvas
 */
function applyThermal(srcVideo, destCanvas) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    // Bayer-ordered dither offset before quantising to 8 steps
...
// Key upper-body pose connections
const POSE_CONNECTIONS = [[11,12],[11,13],[13,15],[12,14],[14,16],[11,23],[12,24]];

/**
 * Draw MediaPipe Holistic skeleton (hands + pose) onto a canvas context.
 *
