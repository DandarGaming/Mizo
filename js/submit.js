...
let submitHolistic     = null;   // separate Holistic instance for submit panel
...
let capturedSeq        = null;   // { keypoints: float[][], frames: int }
let recordBuffer       = [];
const RECORD_TARGET    = 60;     // frames to capture per sign
...
/**
 * Close the submit panel.
 * When called from the overlay's onclick, only closes if the click
 * hit the backdrop itself (not a child element).
 */
...
/** Open the camera inside the submit panel. */
async function startSubmitCamera() {
  ...
  try {
    ...
    // Initialise a separate Holistic instance for the submit panel
...
/** Release the submit-panel camera and stop the render loop. */
function stopSubmitCamera() {
  ...
  // Reset recording state if mid-recording
  isRecording = false;
  document.getElementById('recBtn').textContent = '● RECORD (60 FRAMES)';
  document.getElementById('recBtn').classList.remove('recording');
}

/** Per-frame render loop for the submit panel. */
async function submitRenderLoop() {
  ...
  if (video.readyState === video.HAVE_ENOUGH_DATA) {
    ...
    if (isRecording) {
      ...
      const circ     = 283;
      document.getElementById('recRingBar').style.strokeDashoffset =
        circ - circ * progress;
      document.getElementById('submitFrameBadge').textContent =
        recordFramesDone + '/' + RECORD_TARGET;
...
function finishRecording() {
  isRecording = false;
  capturedSeq = {
    keypoints: recordBuffer.slice(0, RECORD_TARGET),
    frames:    RECORD_TARGET,
  };
  recordBuffer = [];
...
function previewCapture() {
  ...
  showToast(
    'PREVIEW: ' + capturedSeq.frames + ' FRAMES · ' +
    capturedSeq.keypoints[0].length + ' FEATURES EACH'
  );
}
...
function submitSign() {
  ...
  const submission = {
    ...
    // Strip any HTML tags from free-text notes
    notes:      document.getElementById('signNotes').value
...
function renderQueue() {
  ...
  [...submissionQueue].reverse().forEach(sub => {
    ...
    // Action buttons — sub.id is a timestamp number, safe to inline
...
/** Sanitise a label for use in a filename. */
function safeName(n) {
  return String(n).replace(/[^A-Z0-9_\-]/gi, '_').slice(0, 40);
}

/**
 * Return a plain-object copy of a submission with only whitelisted fields,
 * preventing prototype pollution in exports.
 */
...
function downloadSingle(id) {
  ...
  const blob = new Blob([JSON.stringify(sanitiseSub(sub), null, 2)], {
...
function exportAllSubmissions() {
  ...
  const payload = {
    ...
    count:       submissionQueue.length,
    submissions: submissionQueue.map(sanitiseSub),
  };
